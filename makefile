...
build-linux:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 $(GOBUILD) -o $(BUILD_BASE_DIR)/$(BINARY_NAME)-linux-amd64
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 $(GOBUILD) -o $(BUILD_BASE_DIR)/$(BINARY_NAME)-linux-arm64

# Builds the project for Windows
build-windows:
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 $(GOBUILD) -o $(BUILD_BASE_DIR)/$(BINARY_NAME)-windows-amd64.exe
	CGO_ENABLED=0 GOOS=windows GOARCH=arm64 $(GOBUILD) -o $(BUILD_BASE_DIR)/$(BINARY_NAME)-windows-arm64.exe

# Builds the project for macOS (Darwin)
build-darwin:
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 $(GOBUILD) -o $(BUILD_BASE_DIR)/$(BINARY_NAME)-darwin-amd64
	CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 $(GOBUILD) -o $(BUILD_BASE_DIR)/$(BINARY_NAME)-darwin-arm64
